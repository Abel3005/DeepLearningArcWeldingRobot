...
#    e_p_array1 = np.array([[pose.poses[0].pose.position.x], [pose.poses[0].pose.position.y], [pose.poses[0].pose.position.z],[1]])
  ...
  def go_to_target_temp(self, target):
    ...
    pose_goal.position.x =  target[0][3] + 0.07 +0.03            #- 0.402794
    pose_goal.position.y =  target[1][3] + 0.035             #- 0.002158351 
    pose_goal.position.z =  target[2][3] - 0.05       #+0.1  #- 0.450 #+ 0.156928  #+450 : 팬던트 상에서는 450mm가 더해져서 나오기때문에 코드상에서는 더해줘야함.
...
# Degree y:19' 
    h_e_array = np.array([  [0.9563047,  0.0000000, 0.2923717,  0.112 ], 
                            [0.0000000,  1.0000000,  0.0000000, -0.025],
                            [-0.2923717,  0.0000000,  0.9563047,  0.153 ],
                            [0         ,  0        ,  0        , 1]])
